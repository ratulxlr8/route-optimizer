...
A real-time shipping rate calculator ("Smart Courier Auto-Splitter") for Bangladeshi F-commerce/e-commerce merchants. Enter one order (delivery zone, weight, price, COD) — or bulk-upload a CSV or Excel file of orders — and instantly compare **Pathao**, **RedX**, **CarryBee**, and **Steadfast** rates side by side, with the cheapest option highlighted.

## SEO / AEO / PWA

- `lib/site.ts` — single source of truth for the production domain (`SITE_URL`), brand name, and the common phonetic/spelling variants (Fleet Split, Flitspit, ফ্লিটস্প্লিট) surfaced in visible FAQ copy and structured data rather than stuffed into meta tags.
- `app/layout.tsx` — full metadata (title template, OpenGraph, Twitter card, canonical, robots) plus `components/structured-data.tsx` (Organization + WebApplication JSON-LD).
- `app/robots.ts`, `app/sitemap.ts` — generated at build time from the same route list.
- `app/faq/page.tsx`, `app/about/page.tsx`, and four courier-specific landing pages (`app/pathao-delivery-charge-calculator/`, `app/redx-delivery-charge-calculator/`, `app/steadfast-courier-charge-calculator/`, `app/carrybee-delivery-charge-calculator/`) — each targets a distinct long-tail query with real rate data pulled from that courier's own pricing module (via `components/courier-landing.tsx`), plus its own `FAQPage` JSON-LD for AEO. FAQ page covers the name-misspelling question directly for both search and AI-answer-engine retrieval.
